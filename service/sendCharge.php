...
if ($chargeDays) {
    foreach ($chargeDays as $subscribe) {
        ...
        if ($transaction) {
            ...
            $subject = "[PAGAMENTO CONFIRMADO] Obrigado por assinar o AppControl";
            $body = $view->render("mail", [
                "subject" => $subject,
                "message" => "<h3>Obrigado {$user->first_name}!</h3><p>Estamos passando apenas para agradecer por você ser um assinante AppControl {$plan->name}.</p><p>Sua fatura deste mês venceu hoje e já está paga de acordo com seu plano. Qualquer dúvida estamos a disposição.</p>"
            ]);
            ...
            $subject = "[ASSINATURA CANCELADA] Sua conta AppControl agora é FREE";
            $body = $view->render("mail", [
                "subject" => $subject,
